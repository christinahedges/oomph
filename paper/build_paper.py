...
def test_paper_lightcurves():
    ...
    ax = c.lc.plot(c='k')
    c.rlc.plot(c='r', ax=ax, lw=0.1, rasterized=True)
    c.blc.plot(c='b', ax=ax, lw=0.1, rasterized=True)
    c.glc.plot(c='g', ax=ax, lw=0.1, rasterized=True)
    ...
    cadence_masks = [(c.lc.time.jd > 54294.9) & (c.lc.time.jd < 54295.6),
                     ...
                     (c.lc.time.jd > 54346.2) & (c.lc.time.jd < 54346.8),
    ...
    with plt.style.context('seaborn-white'):
        ...
        (c.rlc/c.glc).bin(0.04, aggregate_func=np.nanmedian).plot(c='r', ax=ax[0], lw=0.3, rasterized=True)
        (c.blc/c.glc).bin(0.04, aggregate_func=np.nanmedian).plot(c='b', ax=ax[1], lw=0.3, rasterized=True)
        ...
        (c.rlc/c.glc).to_csv('paper/rlc.csv')
        (c.blc/c.glc).to_csv('paper/blc.csv')
        (c.lc).to_csv('paper/lc.csv')
...
def test_run_fit_flares():
    ...
    for idx, cadence_mask in enumerate(tqdm(cadence_masks)):
        fig1, fig2, samples = c.fit_flare(cadence_mask, r_star=3.08*u.solRad)
